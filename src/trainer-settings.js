...
TrainerSettings.resolveSettingsUpdate = function(currentSettings, newSettings) {
  const settingsPatch = typeof newSettings === 'function'
    ? newSettings(currentSettings)
    : newSettings;

  return TrainerSettings.mergeSettings(currentSettings, settingsPatch);
};

TrainerSettings.createPersistentUpdater = function(setSettings, storage, storageKey = 'chordTrainerSettings') {
  return function updatePersistentSettings(newSettings) {
    setSettings(prev => {
      const updatedSettings = TrainerSettings.resolveSettingsUpdate(prev, newSettings);

      if (storage && typeof storage.setItem === 'function') {
        storage.setItem(storageKey, JSON.stringify(updatedSettings));
      }
...
if (typeof window !== 'undefined') {
  window.TrainerSettings = TrainerSettings;
}
